...
//  BlueGecko
//
//  Created by RAVI KUMAR on 20/12/19.
...
extension UIDevice {
    ...
    static func getCurrentDeviceInfo() -> (UUID: String, modelName: String, SystemVersion: String) {
        let UUID: String = UIDevice.current.identifierForVendor?.uuidString ?? ""
        let modelName = UIDevice.current.type.rawValue
        let systemVersion = "\(UIDevice.current.systemName) \(UIDevice.current.systemVersion)"
        return  (UUID: UUID, modelName: modelName, SystemVersion: systemVersion)
    }
    ...
    var type: Model {
        ...
        if let model = modelMap[String.init(validatingUTF8: modelCode!)!] {
            if model == .simulator, let simModelCode = ProcessInfo().environment["SIMULATOR_MODEL_IDENTIFIER"] ,let simModel = modelMap[String.init(validatingUTF8: simModelCode)!] {
                return simModel
            }
            return model
        }
    ...
    var supportsBluetooth5_1: Bool {
        get {
            let supportBluetooth5_1Models: [Model] = [
                .iPhone11,
                .iPhone11Pro,
                .iPhone11ProMax,
                .iPhone12Mini,
            ...
            return supportBluetooth5_1Models.contains(Model(rawValue: self.model) ?? .simulator)
        }
    }
}
