//
//  SILIOPThroughputGaugeView.swift
//  BlueGecko
//
//  Created by SovanDas Maity on 30/03/26.
...
@IBDesignable
class SILIOPThroughputGaugeView: UIView {
...
//    private let endColor = (red: 217.0, green: 30.0, blue: 42.0)
    ...
    private var pendingThroughputResult: SILThroughputResult?
    ...
    private let MaxThroughput = 1_500_000.0
       
    // MARK: - Drawing a gauge view
    
    override func draw(_ rect: CGRect) {
        if animateGaugeLayer != nil {
            applyPendingThroughputResultIfNeeded()
            return
        }
        ...
        drawResultView(center: center, height: 70, width: 100)
        setInitialValue()
        applyPendingThroughputResultIfNeeded()
    }
...
//    }
    ...
    private func drawResultView(center: CGPoint, height: CGFloat, width: CGFloat) {
        let resultView = UIView(frame: CGRect(origin: CGPoint(x: center.x - (width / 2) - 8, y: self.frame.maxY - 100 - 30), size: CGSize(width: width, height: height)))
    ...
    func updateView(throughputResult: SILThroughputResult) {
        pendingThroughputResult = throughputResult
        guard animateGaugeLayer != nil,
              handImageView != nil,
              valueLabel != nil,
              directionImageView != nil,
              throughputUnitLabel != nil else {
              setNeedsDisplay()
            return
        }
        
        applyThroughputResult(throughputResult)
    }
    
    private func applyPendingThroughputResultIfNeeded() {
        guard let pendingThroughputResult = pendingThroughputResult,
              animateGaugeLayer != nil,
              handImageView != nil,
              valueLabel != nil,
              directionImageView != nil,
              throughputUnitLabel != nil else { return }
        
        applyThroughputResult(pendingThroughputResult)
    }
    
    private func applyThroughputResult(_ throughputResult: SILThroughputResult) {
        let percent = min(Double(throughputResult.valueInBits) / MaxThroughput, 1.0)
