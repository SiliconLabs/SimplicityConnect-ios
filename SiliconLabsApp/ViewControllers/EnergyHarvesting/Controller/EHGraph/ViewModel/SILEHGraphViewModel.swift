...
struct SILEHGraphDiscoveredPeripheralData {
    var color: UIColor
...
//Cannot convert value of type 'EHPeripheralData' (aka 'SILEHGraphDiscoveredPeripheralData') to expected argument type 'PeripheralData' (aka 'SILRSSIGraphDiscoveredPeripheralData')
class SILEHGraphViewModel {
    
    private var filterHelper = EHPeripheralDataFilterHelper()
    
    lazy var filter: BehaviorRelay<FilterClosure> = BehaviorRelay(value: filterHelper.getFilterOfType(.none))
    
    lazy var selected: PublishRelay<[EHPeripheralData]> = PublishRelay()
    
    lazy var refresh = PublishRelay<Void>()
    
    private lazy var _peripherals = BehaviorRelay<[EHPeripheralData]>(value: [])
    
//    lazy var peripherals = Observable.combineLatest(_peripherals, selected.startWith([]), filter)
//        .map { (peripherals, selectedPeripherals, filter) -> [EHPeripheralData] in
//            // filter
...
//
    ...
    private lazy var newPeripherals: Observable<EHPeripheralData> = centralManager
        .newDiscoveredPeripheral
        .map {
            EHPeripheralData(color: EHRSSIConstants.lineColor(), peripheral: $0, isSelected: false, uuid: $0.identityKey,
    ...
    private let centralManager: SILEHGraphCentralManager
    ...
    init() {
        self.centralManager = SILEHGraphCentralManager()
