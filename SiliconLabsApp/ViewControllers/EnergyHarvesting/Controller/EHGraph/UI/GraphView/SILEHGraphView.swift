...
class SILEHGraphView: UIView {
    
    private var referenceDate = Date()
    
    var refresh: PublishRelay<Void> = PublishRelay()
    var input: PublishRelay<[SILEHGraphDiscoveredPeripheralData]> = PublishRelay()

    private var disposeBag = DisposeBag()
    
    @IBOutlet weak var contentView: UIView!
    @IBOutlet weak var chartView: EHGraphLineChartView! // renamed from RSSIGraphLineChartView
    
    private let rightArrowButton = SILBigButton()
    private let leftArrowButton = SILBigButton()

    private var minimumYValue: Double = EHRSSIConstants.startYAxisMinimum
    private var maximumYValue: Double = EHRSSIConstants.startYAxisMaximum
    ...
    private func commonInit() {
        Bundle.main.loadNibNamed("SILEHGraphView", owner: self, options: nil)
    ...
    private func setupInput() {
        input.asObservable()
            ...
            .subscribe(onNext: { [weak self] cellData in
                ...
                if let dataSet = self.chartView.lineData?.dataSets.first(where: { $0.label == cellData.uuid }) as? LineChartDataSet {
                    dataSet.setColor(cellData.color)
                    dataSet.lineWidth = cellData.isSelected ? EHRSSIConstants.selectedLineWidth : EHRSSIConstants.unselectedLineWidth
                    if cellData.isSelected {
                        self.chartView.lineData?.removeDataSet(dataSet)
                        self.chartView.lineData?.append(dataSet)
                    }
                }
            })
        ...
        input.asObservable().observe(on: ConcurrentDispatchQueueScheduler(qos: .userInitiated))
            ...
            .subscribe(onNext: { [weak self] val in
                ...
                self.addOrUpdateDataForPeripheral(id, measurement: measurement, withColor: color)
            })
            .disposed(by: disposeBag)
        //
    ...
    func refreshGraph() {
        ...
        chartView.xAxis.axisMinimum = EHRSSIConstants.startXAxisMinimum
        chartView.xAxis.axisMaximum = max(now, EHRSSIConstants.maxNumberOfVisibleXValues)
        
        chartView.leftAxis.axisMinimum = minimumYValue
        chartView.leftAxis.axisMaximum = maximumYValue
        chartView.setVisibleXRangeMinimum(EHRSSIConstants.minNumberOfVisibleXValues)
        chartView.setVisibleXRangeMaximum(EHRSSIConstants.maxNumberOfVisibleXValues)
        chartView.setVisibleYRangeMinimum(EHRSSIConstants.minNumberOfVisibleYValues, axis: .left)
        chartView.setVisibleYRangeMaximum(EHRSSIConstants.maxNumberOfVisibleYValues, axis: .left)
        ...
        rightArrowButton.isHidden = !(chartView.highestVisibleX + EHRSSIConstants.approximationError  < axisMaximum)
    ...
    private func setupRightArrowButton() {
        ...
        rightArrowButton.extendLeft = EHRSSIConstants.extendedButtonOffset
    ...
    private func setupLeftArrowButton() {
        ...
        leftArrowButton.extendRight = EHRSSIConstants.extendedButtonOffset
    ...
    func resetGraphCompletely() {
        referenceDate = Date()
        minimumYValue = EHRSSIConstants.startYAxisMinimum
        maximumYValue = EHRSSIConstants.startYAxisMaximum
...
struct EHRSSIConstants {
    static let axisBlack = UIColor.lightGray
    ...
    static let minNumberOfVisibleYValues: Double = 500
    static let maxNumberOfVisibleYValues: Double = 5000

    static let unselectedLineWidth = 2.0
    static let selectedLineWidth = 3.0
    static let circleRadious = 2.1
    
    static let startYAxisMinimum = 0.0
    static let startYAxisMaximum = 5000.0
    static let yAxisGranularity = 500.0
...
extension SILEHGraphView {
    func addRandomVoltagePoint(identifier: String = "Voltage", color: UIColor = .systemBlue, voltageValue: Double) {
        DispatchQueue.main.async { [weak self] in guard let self = self else { return }
            ...
            let x = Date().timeIntervalSince(self.referenceDate)
//            let y = Double.random(in: 2000...3000)  // voltage range (mV)
            let y = voltageValue
            let entry = ChartDataEntry(x: x, y: y)
            if let ds = self.chartView.lineData?.dataSets.first(where: { $0.label == identifier }) as? LineChartDataSet { ds.append(entry)
            } else { self.chartView.addDataSetFor([entry], identifier: identifier, color: color) }
            self.maximumYValue = max(self.maximumYValue, y)
            self.minimumYValue = min(self.minimumYValue, y)
            self.refreshGraph()
        }
    }
}
