//
//  SILWifiThroughputGaugeView.swift
//  SiliconLabsApp
//
...
enum SILWifiThroughputTestType {
    case wifiIndications
    case wifiNotifications
    case wifiNone
}
enum SILWiFiThroughputTestDirection {
    case wifiPhoneToEFR
    case wifiEFRToPhone
    case wifiNone
}

struct SILWifiThroughputResult {
    var wifiSender: SILWiFiThroughputTestDirection
    var wifiTestType: SILWifiThroughputTestType
    var wifiValueInBits: Int
}
class SILWifiThroughputGaugeView: UIView {
...
//                          "16Mbps"]
    ...
    private let MaxThroughput = 100.00
    ...
    func updateView(throughputResult: SILWifiThroughputResult) {
        let percent = Double(throughputResult.wifiValueInBits) / MaxThroughput
        updateResultView(throughputResult: throughputResult)
        numberOfSelectedSegments = Int(percent * Double(numberOfSegments))
    }

    private func updateResultView(throughputResult: SILWifiThroughputResult) {
        valueLabel.text = "\(throughputResult.wifiValueInBits).0"
        switch throughputResult.wifiSender {
        case .wifiEFRToPhone:
            directionImageView.image = directionEFRPhoneImage
