...
class SILCreateGattCharacteristicViewModel {
    ...
    private var fixedVariableLength: Bool = false
    private var characteristicInitialValueTypeOnStartEdiditing: SILGattConfigurationValueType?
    ...
    init(wireframe: SILGattConfiguratorDetailsWireframe, repository: SILGattAssignedNumbersRepository, characteristic: SILGattConfigurationCharacteristicEntity? = nil, onSave: @escaping (SILGattConfigurationCharacteristicEntity) -> ()) {
        ...
        if let characteristic = characteristic {
            ...
            self.fixedVariableLength = characteristic.fixedVariableLength
            self.characteristicInitialValueTypeOnStartEdiditing = self.characteristicInitialValueType
        } else {
    ...
    func onSave() {
        ...
        if let characteristic = self.characteristic {
            save(gattCharacteristic: characteristic)
        } else {
            save()
        }
        wireframe.dismissPopover()
    ...
    private func save(gattCharacteristic: SILGattConfigurationCharacteristicEntity? = nil) {
        var gattCharacteristic = gattCharacteristic
        if gattCharacteristic == nil {
            gattCharacteristic = SILGattConfigurationCharacteristicEntity()
        }
        ...
        gattCharacteristic!.fixedVariableLength = self.fixedVariableLength
        if gattCharacteristic!.initialValueType != self.characteristicInitialValueTypeOnStartEdiditing {
            gattCharacteristic!.fixedVariableLength = false
            if let lengthAttributeIndex = gattCharacteristic!.additionalXmlAttributes.firstIndex(where: { attribute in attribute.name == "value_length" }) {
                gattCharacteristic!.additionalXmlAttributes.remove(at: lengthAttributeIndex)
            }
        }
